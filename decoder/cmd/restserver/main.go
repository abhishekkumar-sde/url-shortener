...
import (
	...
	"url-shortener/decoder/dl"
	"url-shortener/decoder/endpoint"
	"url-shortener/decoder/inithandler"
	"url-shortener/decoder/svcparam/envvar"
	httptransport "url-shortener/decoder/transport/http"
...
func main() {
	...
	decoderBL := bl.NewDecoderBL(repo, cache, viper.GetString(envvar.BaseURL))
	endpoint := endpoint.NewURLEndpoint(decoderBL, limiter)
